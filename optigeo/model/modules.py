...
from .dinov3.hub.backbones import dinov3_vit7b16, dinov3_vitl16, _make_dinov3_vit, Weights
...
class DINOv3Encoder(nn.Module):
    ...
    def __init__(self, intermediate_layers: Union[int, List[int]], dim_out: int):
        super(DINOv3Encoder, self).__init__()

        # self.backbone = dinov3_vit7b16(
        #     pretrained=False, 
        # )
        # self.backbone._name = "dinov3_vit7b16"
        ...
        self.backbone._name = "dinov3_vitl16"
    ...
    def patch_size(self) -> int:
        return int(self.backbone.patch_size) # 16

    def init_weights_from(self, ckpt_path, device: str = "cuda"):
        ckpt = torch.load(ckpt_path, map_location=device)
        sd = ckpt.get('model', ckpt.get('state_dict', ckpt))
        res = self.backbone.load_state_dict(sd, strict=False)
        print("DINOv3 backbone load - missing keys: ", res.missing_keys, "; unexpected keys: ", res.unexpected_keys)
    ...
    def forward(self, image: torch.Tensor, token_rows: Union[int, torch.LongTensor], token_cols: Union[int, torch.LongTensor], return_class_token: bool = False) -> Tuple[torch.Tensor, torch.Tensor]:
        image_16 = F.interpolate(image, (token_rows * 16, token_cols * 16), mode="bilinear", align_corners=False, antialias=True)
