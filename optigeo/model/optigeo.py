...
from . import DEFAULT_PRETRAINED_FILENAME
...
class OptiGeo(nn.Module):
    ...
    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path: Union[str, Path, IO[bytes]], model_kwargs: Optional[Dict[str, Any]] = None, **hf_kwargs) -> 'OptiGeo':
        """
        ...
        ### Returns:
        - A new instance of `OptiGeo` with the parameters loaded from the checkpoint.
        """
        if Path(pretrained_model_name_or_path).exists():
            ...
            cached_checkpoint_path = hf_hub_download(
                ...
                repo_type="model",
                filename=hf_kwargs.pop("filename", DEFAULT_PRETRAINED_FILENAME),
                **hf_kwargs
            )
