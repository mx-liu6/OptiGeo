...
def safe_norm(x: torch.Tensor, dim: int = -1, eps: float = 1e-8):
    return torch.sqrt(torch.clamp((x * x).sum(dim=dim, keepdim=True), min=eps))

def normalize(x: torch.Tensor, dim: int = -1, eps: float = 1e-8):
    return x / safe_norm(x, dim=dim, eps=eps)

def build_tangent_basis(r: torch.Tensor, eps:float = 1e-8):
    """
    r: (B, H, W, 3) unit vectors
    returns b1 b2: (B, H, W, 3), orthonormal, both orthogonal to r
    Robust: choose reference axis a depending on r to avoid near-parallel cross product
    """
    device, dtype = r.device, r.dtype
    # candidate axes
    a1 = torch.tensor([0.0, 1.0, 0.0], device=device, dtype=dtype).view(1, 1, 1, 3) # up vector
    a2 = torch.tensor([0.0, 0.0, 1.0], device=device, dtype=dtype).view(1, 1, 1, 3) # forward vector

    # if [r*a1] is too large -> r ~ y-axis, then use z-axis
    # dot: (B, H, W, 1)
    dot1 = (r * a1).sum(dim=-1, keepdim=True).abs()
    use_a2 = (dot1 > 0.95).to(dtype=dtype)
    a = a1 * (1.0 - use_a2) + a2 * use_a2

    b1 = torch.cross(a.expand_as(r), r, dim=-1)
    b1 = normalize(b1, dim=-1, eps=eps)
    b2 = torch.cross(r, b1, dim=-1)
    b2 = normalize(b2, dim=-1, eps=eps)
    return b1, b2

def apply_delta_to_ray(
    r: torch.Tensor,
    delta1: torch.Tensor,
    delta2: torch.Tensor,
    delta_max_rad: float = 0.0523598, # 3 deg in rad by default
    eps: float = 1e-8,
):
    """
    r: (B,H,W,3) unit
    delta1/delta2: (B,H,W,1) raw (ubounded) 
    return r': (B,H,W,3) unit
    """
    # bound deltas to small angular perturbations
    d1 = delta_max_rad * torch.tanh(delta1)
    d2 = delta_max_rad * torch.tanh(delta2)

    # build tangent basis
    b1, b2 = build_tangent_basis(r, eps=eps)

    # apply deltas
    r_tilde = r + d1 * b1 + d2 * b2
    r_prime = normalize(r_tilde, dim=-1, eps=eps)
