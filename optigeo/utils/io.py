...
def read_image(path: Union[str, os.PathLike, IO]) -> np.ndarray:
    ...
    """
    if cv2 is None:
        raise ModuleNotFoundError("cv2 is required for read_image; please install opencv-python or opencv-python-headless.")
...
def write_image(path: Union[str, os.PathLike, IO], image: np.ndarray, quality: int = 95):
    ...
    """
    if cv2 is None:
        raise ModuleNotFoundError("cv2 is required for write_image; please install opencv-python or opencv-python-headless.")
...
def read_normal(path: Union[str, os.PathLike, IO]) -> np.ndarray:
    ...
    """
    if cv2 is None:
        raise ModuleNotFoundError("cv2 is required for read_normal; please install opencv-python or opencv-python-headless.")
    if isinstance(path, (str, os.PathLike)):
        data = Path(path).read_bytes()
    else:
        data = path.read()
...
def write_normal(path: Union[str, os.PathLike, IO], normal: np.ndarray, compression_level: int = 7) -> np.ndarray:
    ...
    """
    if cv2 is None:
        raise ModuleNotFoundError("cv2 is required for write_normal; please install opencv-python or opencv-python-headless.")
