...
from optigeo.train.dataloader import TrainDataLoaderPipeline
from optigeo.train.losses import (
    affine_invariant_global_loss,
    affine_invariant_local_loss,
    scalefield_invariant_global_loss,
    edge_loss,
    normal_loss, 
    mask_l2_loss, 
    mask_bce_loss,
    monitoring, 
    metric_l1_loss_xyz,
    metric_scalefield_loss,
    metric_ray_direction_loss,
    metric_scalefield_range_loss,
    delta_reg_loss,
)
from optigeo.train.utils import build_optimizer, build_lr_scheduler
from optigeo.utils.geometry_torch import intrinsics_to_fov
from optigeo.utils.vis import colorize_depth, colorize_normal, colorize_scalar_field, colorize_vector_field
from optigeo.utils.tools import key_average, recursive_replace, CallbackOnException, flatten_nested_dict
...
@click.option('--freeze_encoder', type=bool, default=False, help='Freeze the encoder')
...
def main(
    ...
    with accelerator.local_main_process_first():
        ...
        OptiGeo = import_model_class_by_version(config.get('model_version'))
        model = OptiGeo(**config['model'])
    count_total_parameters = sum(p.numel() for p in model.parameters())
    print(f'Total parameters: {count_total_parameters}')

    # Show stats
    num_backbone = sum(param.numel() for param in model.backbone.parameters())
    num_backbone_trainable = sum(param.numel() for param in model.backbone.parameters() if param.requires_grad)
    num_head_trainable = sum(param.numel() for param in model.head.parameters() if param.requires_grad)
    print(f'Backbone trainable: {num_backbone_trainable}/{num_backbone}')
    print(f'Head trainable: {num_head_trainable}')
    ...
    if enable_gradient_checkpointing and not config['model']['freeze_encoder']:
    ...
    if checkpoint is None:
        ...
        with accelerator.local_main_process_first():
            model.init_weights(config['model']['dinov3_ckpt'])
        initial_step = 0
    else:
    ...
    if torch.version.hip and isinstance(model, torch.nn.parallel.DistributedDataParallel):
        ...
        from optigeo.model.utils import sync_ddp_hook
    ...
    with (
        ...
        for i_step in range(initial_step, num_iterations):
            ...
            while i_accumulate < gradient_accumulation_steps:
                # Load batch
                # Debug prints: only emit on the first step + first micro-batch, per rank.
                do_debug_print = (i_step == initial_step and i_accumulate == 0)
                if do_debug_print:
                    print(
                        f"[rank {accelerator.process_index}] before train_data_pipe.get() | i_step={i_step} i_accumulate={i_accumulate}",
                        flush=True,
                    )
                batch = train_data_pipe.get()
                if do_debug_print:
                    # Keep output short: only label distribution.
                    _labels = batch.get("label_type", [])
                    _label_set = sorted(set(_labels))
                    _label_counts = {lb: sum(1 for x in _labels if x == lb) for lb in _label_set}
                    print(
                        f"[rank {accelerator.process_index}] after train_data_pipe.get() | label_counts={_label_counts}",
                        flush=True,
                    )
                ...
                # NOTE: Do NOT `continue` here in distributed training.
                # If some ranks skip forward/backward while others don't, DDP/NCCL collectives can desync and hang.
                # We still compute a valid (zero-gradient) loss below to keep all ranks aligned.
                if do_debug_print:
                    _invalid_count = sum(1 for lb in label_type if lb == "invalid")
                    _is_metric_counts = {True: sum(1 for x in is_metric if x), False: sum(1 for x in is_metric if not x)}
                    print(
                        f"[rank {accelerator.process_index}] tensors ready | current_batch_size={current_batch_size} invalid_count={_invalid_count} is_metric_counts={_is_metric_counts}",
                        flush=True,
                    )
                ...
                with accelerator.accumulate(model):
                    ...
                    if do_debug_print:
                        print(f"[rank {accelerator.process_index}] start forward | num_tokens={num_tokens}", flush=True)
                    ...
                    pred_scalefield = output.get('scalefield', None)
                    pred_points_metric = pred_points * pred_scalefield
                    pred_delta = output.get('delta', None)
                    if do_debug_print:
                        print(f"[rank {accelerator.process_index}] end forward | pred_points.shape={tuple(pred_points.shape)}", flush=True)
                    ...
                    for i in range(current_batch_size):
                        ...
                        for k, v in config['loss'][label_type[i]].items():
                            ...
                            elif v['function'] == 'metric_l1_loss_xyz':
                                if is_metric[i]:
                                    loss_dict[k], misc_dict[k] = metric_l1_loss_xyz(pred_points_metric[i], gt_points[i], gt_mask[i], **v['params'])
                                else:
                                    continue
                            elif v['function'] == 'scalefield_invariant_global_loss':
                                if is_metric[i]:
                                    loss_dict[k], misc_dict[k] = scalefield_invariant_global_loss(pred_points[i], gt_points[i], gt_mask[i], **v['params'])
                                else:
                                    continue
                            elif v['function'] == 'metric_scalefield_loss':
                                if is_metric[i]:
                                    loss_dict[k], misc_dict[k] = metric_scalefield_loss(pred_points[i], pred_scalefield[i], gt_points[i], gt_mask[i], **v['params'])
                                else:
                                    continue
                            elif v['function'] == 'metric_ray_direction_loss':
                                if is_metric[i]:
                                    loss_dict[k], misc_dict[k] = metric_ray_direction_loss(pred_points[i], gt_points[i], gt_mask[i], **v['params'])
                                else:
                                    continue
                            elif v['function'] == 'metric_scalefield_range_loss':
                                if is_metric[i]:
                                    if pred_scalefield is not None:
                                        loss_dict[k], misc_dict[k] = metric_scalefield_range_loss(pred_points[i], pred_scalefield[i], gt_points[i], gt_mask[i], **v['params'])
                                else:
                                    continue
                            elif v['function'] == 'delta_reg_loss':
                                if is_metric[i]:
                                    if pred_delta is not None:
                                        loss_dict[k], misc_dict[k] = delta_reg_loss(pred_delta[i], gt_mask[i], **v['params'])
                                else:
                                    continue
                            else:
                        ...
                        # Ensure the loss always depends on model outputs (even if `invalid` has no configured losses).
                        # This guarantees backward produces gradients (all-reduce participates) and avoids DDP/NCCL desync.
                        loss_ = sum([weight_dict[k] * loss_dict[k] for k in loss_dict], start=pred_points[i].sum() * 0.0)
                    ...
                    # Backward & update
                    if do_debug_print:
                        # Print only one scalar to keep logs small.
                        print(f"[rank {accelerator.process_index}] before backward | loss={loss.item()}", flush=True)
                    accelerator.backward(loss)
                    if do_debug_print:
                        print(f"[rank {accelerator.process_index}] after backward | sync_gradients={accelerator.sync_gradients}", flush=True)
            ...
            if vis_every > 0 and accelerator.is_main_process and (i_step == initial_step or i_step % vis_every == 0):
                ...
                with torch.inference_mode():
                    for i_batch, batch in enumerate(tqdm(batches_for_vis, desc=f'Visualize: {i_step:08d}', leave=False)):
                        ...
                        pred_points_metric = output['points_metric'].cpu().numpy() # Metric version
                        pred_depth_metric = output['depth_metric'].cpu().numpy() # Metric version
                        pred_scalefield = output['scalefield'].cpu().numpy() if 'scalefield' in output else None
                        pred_delta = output['delta'].cpu().numpy() if 'delta' in output else None

                        pred_normal_t, pred_normal_mask_t = utils3d.torch.points_to_normals(output['points'], output['mask'])
                        pred_normal = pred_normal_t.cpu().numpy()
                        ...
                        for i_instance in range(image.shape[0]):
                            ...
                            pred_points_metric_i = pred_points_metric[i_instance]
                            pred_depth_metric_i = pred_depth_metric[i_instance]
                            pred_normal_i = pred_normal[i_instance]
                            pred_scalefield_i = pred_scalefield[i_instance] if pred_scalefield is not None else None
                            pred_delta_i = pred_delta[i_instance] if pred_delta is not None else None
                            save_dir.joinpath(f'{idx:04d}').mkdir(parents=True, exist_ok=True)
                            cv2.imwrite(str(save_dir.joinpath(f'{idx:04d}/image.jpg')), cv2.cvtColor(image_i, cv2.COLOR_RGB2BGR))
                            ...
                            cv2.imwrite(str(save_dir.joinpath(f'{idx:04d}/points_metric.exr')), cv2.cvtColor(pred_points_metric_i, cv2.COLOR_RGB2BGR), [cv2.IMWRITE_EXR_TYPE, cv2.IMWRITE_EXR_TYPE_FLOAT])
                            cv2.imwrite(str(save_dir.joinpath(f'{idx:04d}/depth_metric_vis.png')), cv2.cvtColor(colorize_depth(pred_depth_metric_i, pred_mask_i), cv2.COLOR_RGB2BGR))
                            cv2.imwrite(str(save_dir.joinpath(f'{idx:04d}/normal.png')), cv2.cvtColor(colorize_normal(pred_normal_i), cv2.COLOR_RGB2BGR))
                            if pred_scalefield_i is not None:
                                cv2.imwrite(
                                    str(save_dir.joinpath(f'{idx:04d}/scalefield.png')),
                                    cv2.cvtColor(colorize_scalar_field(pred_scalefield_i[..., 0], pred_mask_i), cv2.COLOR_RGB2BGR)
                                )
                            if pred_delta_i is not None:
                                delta_magnitude_i = np.linalg.norm(pred_delta_i, axis=-1)
                                cv2.imwrite(
                                    str(save_dir.joinpath(f'{idx:04d}/delta_mag.png')),
                                    cv2.cvtColor(colorize_depth(delta_magnitude_i, pred_mask_i), cv2.COLOR_RGB2BGR)
                                )
                                cv2.imwrite(
                                    str(save_dir.joinpath(f'{idx:04d}/delta_vis.png')),
                                    cv2.cvtColor(colorize_vector_field(pred_delta_i, pred_mask_i), cv2.COLOR_RGB2BGR)
                                )
