...
def main(share: bool, pretrained_model_name_or_path: str, use_fp16: bool):
    ...
    import utils3d
    from optigeo.utils.io import write_normal
    ...
    from optigeo.utils.tools import timeit

    print("Load model...")
    if pretrained_model_name_or_path is None:
        pretrained_model_name_or_path = DEFAULT_PRETRAINED_MODEL
    model = import_model_class_by_version().from_pretrained(pretrained_model_name_or_path).cuda().eval()
    if use_fp16:
        model.half()
    ...
    def run(image: np.ndarray, max_size: int = 800, resolution_level: str = 'High',  apply_mask: bool = True, remove_edge: bool = True, request: gr.Request = None):
        ...
        tempdir = Path(tempfile.gettempdir(), 'optigeo')
...
f'''
<div align="center">
<h1> Turn a 2D image into 3D with OptiGeo <a title="Github" href="https://github.com/microsoft/OptiGeo" target="_blank" rel="noopener noreferrer" style="display: inline-block;"> <img src="https://img.shields.io/github/stars/microsoft/OptiGeo?label=GitHub%20%E2%98%85&logo=github&color=C8C" alt="badge-github-stars"> </a> </h1>
</div>
''')
