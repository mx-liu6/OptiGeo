...
@click.option('--pretrained', 'pretrained_model_name_or_path', type=str, default='mxliu-hku/OptiGeo', help='Pretrained model name or path. Defaults to "mxliu-hku/OptiGeo".')
...
def main(
    ...
    import utils3d
    from optigeo.model.optigeo import OptiGeo
    from optigeo.utils.io import save_glb, save_ply
    from optigeo.utils.vis import colorize_depth
    from optigeo.utils.panorama import spherical_uv_to_directions, get_panorama_cameras, split_panorama_image, merge_panorama_depth
    ...
    model = OptiGeo.from_pretrained(pretrained_model_name_or_path).to(device).eval()
