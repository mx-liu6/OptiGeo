...
@click.option('--fov_x', 'fov_x_', type=float, default=None, help='If camera parameters are known, set the horizontal field of view in degrees. Otherwise, optigeo will estimate it.')
...
@click.option('--threshold', type=float, default="0.03", help='Threshold for removing edges. Defaults to 0.01. Smaller value removes more edges. "inf" means no thresholding.')
...
@click.option('--save_in_image_parent', is_flag=True, help='Save outputs directly in each image parent directory instead of creating a subfolder named after the image stem.')
def main(
    input_path: str,
    fov_x_: float,
    output_path: str,
    ...
    from optigeo.model import DEFAULT_PRETRAINED_MODEL, import_model_class_by_version
    from optigeo.utils.io import save_glb, save_ply
    from optigeo.utils.vis import colorize_depth, colorize_normal
    import utils3d
    ...
    if pretrained_model_name_or_path is None:
        pretrained_model_name_or_path = DEFAULT_PRETRAINED_MODEL
    model = import_model_class_by_version().from_pretrained(pretrained_model_name_or_path).to(device).eval()
    if use_fp16:
        model.half()
    ...
    for image_path in (pbar := tqdm(image_paths, desc='Inference', disable=len(image_paths) <= 1)):
        ...
        if 'normal' in output:
            ...
            normal, _ = utils3d.torch.points_to_normals(torch.from_numpy(points), torch.from_numpy(mask))
        ...
        if save_in_image_parent:
            save_path = Path(output_path, relative_parent)
        else:
            save_path = Path(output_path, relative_parent, image_path.stem)
