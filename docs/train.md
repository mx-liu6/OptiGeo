...
This document provides instructions for training and finetuning the OptiGeo model.

## Requirements

Training and finetuning dependencies are listed in [`pyproject.toml`](../pyproject.toml) and [`requirements.txt`](../requirements.txt). The most important training packages are:
...
│   │   ├── image.jpg   # RGB image.
│   │   ├── depth.png   # 16-bit depth. See optigeo/utils/io.py for details
│   │   ├── meta.json   # Stores "intrinsics" as a 3x3 matrix
│   │   └── ...         # Other components such as segmentation mask, normal map etc.
...
```
...
* For depth images, it is recommended to use `read_depth()` and `write_depth()` in [`optigeo/utils/io.py`](../optigeo/utils/io.py) to read and write depth images. The depth is stored in logarithmic scale in 16-bit PNG format, offering a balanced precision, dynamic range and compression ratio compared to 16-bit and 32-bit EXR and linear depth formats. It also encodes `NaN` and `Inf` values for invalid depth values.
...
* We also support reading and storing segmentation masks for evaluation data (see paper evaluation of local points), which are saved in PNG format with semantic labels stored in PNG metadata as JSON strings. See `read_segmentation()` and `write_segmentation()` in [`optigeo/utils/io.py`](../optigeo/utils/io.py) for details.
...
```bash
python optigeo/scripts/vis_data.py PATH_TO_INSTANCE --ply [-o SOMEWHERE_ELSE_TO_SAVE_VIS]
```

### DataLoader

Our training dataloader is customized to handle loading data, perspective crop, and augmentation in a multithreading pipeline. Please refer to [`optigeo/train/dataloader.py`](../optigeo/train/dataloader.py) for details.


## Configuration

See [`configs/train/OptiGeo.json`](../configs/train/OptiGeo.json) for an example configuration file. The configuration file defines the hyperparameters for training the OptiGeo model.
...
```json
{
    ...
    "model_version": "optigeo",                 # Model version. If you have multiple model variants, you can use this to switch between them.
    "model": {                             # Model hyperparameters. Will be passed to Model __init__() as kwargs.
        "encoder": "dinov3_vits16",
        "dinov3_ckpt": "pretrained/dinov3_vits16_pretrain_lvd1689m-08c60483.pth",
        "remap_output": "exp",
    ...
    "optimizer": {                          # Reflection-like optimizer configurations. See optigeo.train.utils.py build_optimizer() for details.
        "type": "AdamW",
        "params": [
    ...
    "lr_scheduler": {                       # Reflection-like lr_scheduler configurations. See optigeo.train.utils.py build_lr_scheduler() for details.
        "type": "SequentialLR",
        "params": {
            "schedulers": [
...
Launch the training script [`optigeo/scripts/train.py`](../optigeo/scripts/train.py). Note that we use [`accelerate`](https://github.com/huggingface/accelerate) for distributed training.
...
To finetune the pre-trained OptiGeo model, download the model checkpoint and put it in a local directory, e.g. `pretrained/OptiGeo-vitl.pt`.

> NOTE: when finetuning pretrained OptiGeo model, a much lower learning rate is required. 
...
accelerate launch \
    ...
    --gradient_accumulation_steps 2 \
    --batch_size_forward 2 \
    --checkpoint pretrained/OptiGeo-vitl.pt \
    --enable_gradient_checkpointing True \
    --vis_every 1000 \
    --enable_mlflow True
```
